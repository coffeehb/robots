...
# scrapers
import hideMyAss_scraper
import proxy_list_org_scraper
#import free_proxy_list_net_scraper
import samair_scraper
...
class Collectioner(threading.Thread):
    ...
    def collect(self):
        # Uses itself if scraped proxies are available. If not, no proxies are used while scrapeing
        proxyHarvesterObj = proxyhandler.ProxyHarvester('proxies.p') 
...
def requestCheck(checkPoint, proxy):
    ...
    bsObj = custom.url2bs(checkPoint, proxies=proxy, timeout=5)
